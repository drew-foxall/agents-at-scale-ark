...
func TestConvertA2AMessagesToCompat(t *testing.T) {
	messages := []protocol.Message{
		...
		protocol.NewMessage(protocol.MessageRoleAgent, []protocol.Part{
			protocol.NewTextPart("response"),
		}),
	}
	compat, err := convertA2AMessagesToCompat(messages)
	require.NoError(t, err)
	require.Len(t, compat, 2)
}

func TestConvertA2AMessagesToCompatMultimodal(t *testing.T) {
	messages := []protocol.Message{
		protocol.NewMessage(protocol.MessageRoleUser, []protocol.Part{
			protocol.NewTextPart("describe this"),
		}),
	}
	compat, err := convertA2AMessagesToCompatMultimodal(messages)
	require.NoError(t, err)
